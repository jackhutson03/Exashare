	    ...
	    <?php if ( $nsfp_query->have_posts() ) : ?>
		    ...
		    <ul>
			    <?php while ( $nsfp_query->have_posts() ) : $nsfp_query->the_post(); ?>
			      <li>
				  <div class="post_image_show_fea"><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
	<img src="<?php the_post_thumbnail_url(); ?>"/>
	</a></div>
	                 <div class="post_info_fea">
					...
					$sum = $wpdb->get_var( 'SELECT meta_value FROM exashare_postmeta WHERE meta_key="rating_avg" and post_id='.$postID);
							...
							?>
                         <div class="fea-rat"><span class="fea-rat-lable"><label>Rating:</label><?php echo $sum;?></span>
						<div class="fea-review-show"><?php the_review_count(); ?></div>
						
								<?php  $networkurl = get_post_meta( get_the_ID(), 'networkurl', true );
								if( $networkurl == ''){ ?>
								<a class="fea-ratimage" href="<?php the_permalink(); ?>"></a>
								<?php } else { ?>
								 <a class="fea-ratimage" href="<?php echo $networkurl ;?>"></a>								
								<?php } ?>
