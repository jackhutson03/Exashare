 ...
 * @return string
 */
function va_list_sort_dropdown( $post_type = '', $base_link = '', $default_current_sort = 'highest_rating' ) {
	global $wp_query;
	...
	$options['highest_rating'] = __( 'Top Rated', APP_TD );
	...
	if ( $post_type == VA_LISTING_PTYPE ) {
		...
		$options['top_reviewed'] = __( 'Top Reviewed', APP_TD );
		$options['newest'] = __( 'Newly Added', APP_TD );
	}
	...
	<script type="text/javascript">
		jQuery('#va_sort_list_<?php echo $post_type; ?>').tinyNav({
			...
			header_href: '<?php echo esc_url( add_query_arg( 'orderby', 'highest_rating' ) ); ?>',
			indent: '-',
...
function va_list_sort_dropdown_content( $post_type = '', $base_link = '', $default_current_sort = 'all_frequencies' ) {
	global $wp_query;
	...
	$options['all_frequencies'] = __( 'All Frequencies', APP_TD );
	if ( $post_type == VA_LISTING_PTYPE ) {
		$options['weekly'] = __( 'Weekly', APP_TD );
		$options['daily'] = __( 'Daily', APP_TD );
		$options['net_60'] = __( 'Net 60', APP_TD );
		$options['net_30'] = __( 'Net 30', APP_TD );
		$options['net_15'] = __( 'Net 15', APP_TD );
	}
	...
	$top_div_control = html( 'div', array( 'class' => 'control' ) );
	$top_div = html( 'div', array( 'class' => 'va_sort_list_selected selected1' ), $top_div_text . $top_div_control );
	...
	<script type="text/javascript">
		jQuery('#va_sort_list_<?php echo $post_type; ?>').tinyNav({
			active: 'active',
			header: '<?php _e( 'Sort Method', APP_TD ); ?>',
			header_href: '<?php echo esc_url( add_query_arg( 'orderby', 'all_frequencies' ) ); ?>',
			indent: '-',
			append: '#va_sort_list_container_<?php echo $post_type; ?>'
		});
	</script>
	...
	return html( 'div', array( 'class' => 'va_sort_list_container', 'id' => 'va_sort_list_container_frq_' . $post_type ), $top_div . $list . $js1 );
...
function va_list_sort_dropdown_content1( $post_type_content = '', $base_link = '', $default_current_sort = 'all_minimum_payout' ) {
	global $wp_query;
	...
	if ( empty( $post_type_content ) ) {
		$post_type_content = $wp_query->get( 'post_type_content' );
		$post_type_content = ! empty( $post_type_content ) ? $post_type_content : VA_LISTING_PTYPE;
	}

	$options['all_minimum_payout'] = __( 'Minimum Payout', APP_TD );
	if ( $post_type_content == VA_LISTING_PTYPE ) {
		//$options['all_minimum_payout '] = __( ' All Minimum Payout' , APP_TD );
		$options['ten'] = __( '10$', APP_TD );
		$options['tw'] = __( '20$', APP_TD );
		$options['th'] = __( '30$', APP_TD );
		$options['fr'] = __( '40$', APP_TD );
		$options['fi'] = __( '50$', APP_TD );
	}
	...
	$top_div_control = html( 'div', array( 'class' => 'control' ) );
	$top_div = html( 'div', array( 'class' => 'va_sort_list_selected selected2' ), $top_div_text . $top_div_control );

	$ul = html( 'ul', array( 'class'=> 'va_sort_list', 'id' => 'va_sort_list_' . $post_type_content ), $li );
	$list = html( 'div', array( 'class' => 'va_sort_list_wrap' ), $ul );

	ob_start();
	?>
	<script type="text/javascript">
		jQuery('#va_sort_list_<?php echo $post_type_content; ?>').tinyNav({
			active: 'active',
			header: '<?php _e( 'Sort Method', APP_TD ); ?>',
			header_href: '<?php echo esc_url( add_query_arg( 'orderby', 'all_minimum_payout' ) ); ?>',
			indent: '-',
			append: '#va_sort_list_container_frq<?php echo $post_type_content; ?>'
		});
	</script>
	<?php
	$js = ob_get_clean();

	return html( 'div', array( 'class' => 'va_sort_list_container', 'id' => 'va_sort_list_container_frq' . $post_type_content ), $top_div . $list . $js );
}