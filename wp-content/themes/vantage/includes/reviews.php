...
function va_process_review( $data ) {
	...
	add_action( 'wp_insert_comment', 'va_set_review_offrer', 11, 3);
	add_action( 'wp_insert_comment', 'va_set_review_payout', 11, 3);
	add_action( 'wp_insert_comment', 'va_set_review_tracking', 11, 3);
	add_action( 'wp_insert_comment', 'va_set_review_support', 11, 3);
	add_action( 'wp_insert_comment', 'va_set_review_payimage', 11, 3);
    add_action( 'wp_insert_comment', 'va_set_reward_rate', 11, 3);
    add_action( 'wp_insert_comment', 'va_set_referral_rate', 11, 3);
	return $data;
}
function va_set_review_payimage  ($review_id,$review) {
if(isset($_FILES['payproof'])){
      $errors= array();
      $file_name = $_FILES['payproof']['name'];
      $file_size =$_FILES['payproof']['size'];
      $file_tmp =$_FILES['payproof']['tmp_name'];
      $file_type=$_FILES['payproof']['type'];
      $file_ext=strtolower(end(explode('.',$_FILES['payproof']['name'])));
      @ $payproo = move_uploaded_file($file_tmp,$_SERVER['DOCUMENT_ROOT'].'/demo/exashare/wp-content/uploads/payproof/'.$file_name);
	  @ update_comment_meta( $review_id, 'payproofimage', "http://webappsample.com/demo/exashare/wp-content/uploads/payproof/".$file_name);
      }
   }
 
function va_set_reward_rate($review_id,$review) {
$rewards = isset( $_POST['reward'] ) ? $_POST['reward'] : 0;
va_set_reward( $review_id, $rewards );
}
function va_set_referral_rate ($review_id,$review) {
$referrals = isset( $_POST['referral_commision'] ) ? $_POST['referral_commision'] : 0;
va_set_referral_commision( $review_id, $referrals );
}

 
function va_set_review_tracking  ($review_id,$review) {
$track = isset( $_POST['tracking_rating'] ) ? $_POST['tracking_rating'] : 0;
va_set_tracking ( $review_id, $track );
}
function va_set_review_support   ($review_id,$review) {
$supp = isset( $_POST['support_rating'] ) ? $_POST['support_rating'] : 0;
va_set_support( $review_id, $supp );
}
function va_set_review_payout ($review_id,$review) {
$payouts = isset( $_POST['payout_rating'] ) ? $_POST['payout_rating'] : 0;
va_set_payout( $review_id, $payouts );
}
function va_set_review_offrer($review_id,$review) {
$offers = isset( $_POST['offers_rating'] ) ? $_POST['offers_rating'] : 0;
va_set_offer( $review_id, $offers );
}
...
function va_set_offer( $review_id, $offnew) {
	return update_comment_meta( $review_id, 'offerating', $offnew );
}
function va_set_payout( $review_id, $paynew) {
	return update_comment_meta( $review_id, 'payrating', $paynew );
}
function va_set_support( $review_id, $supportnew) {
	return update_comment_meta( $review_id, 'supportratnig', $supportnew );
}
function va_set_tracking( $review_id, $trackingnew) {
	return update_comment_meta( $review_id, 'trackingrating', $trackingnew );
}
function va_set_reward( $review_id, $rewardrates) {
	return update_comment_meta( $review_id, 'rewardrate', $rewardrates );
}
function va_set_referral_commision( $review_id, $ratecommisions) {
	return update_comment_meta( $review_id, 'ratecommision', $ratecommisions );
}

/* function va_set_payproofimags( $review_id, $payproo) {
	return update_comment_meta( $review_id, 'payproofimage', $payproo );
} */
