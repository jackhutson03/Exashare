...
<?php if ( have_posts() ) : ?>
	...
	<article class="archive-top listing-archive-top">
		<h3 class="archive-head listing-archive-head"><?php printf( __( 'Network found for "%s"', APP_TD ), va_get_search_query_var( 'ls' )); ?></h3>
	</article>
	<?php endif; ?>
...
<?php else : ?>
	...
	<article class="listing">
		<h2><?php printf( __( 'Sorry, no network were found for "%s"', APP_TD ), va_get_search_query_var( 'ls' )); ?></h2>
	</article>
	<?php elseif ( is_archive() ) : ?>
	<article class="listing">
		<h2><?php printf( __( 'Sorry there are no network for %s "%s"', APP_TD ), ( is_tax( VA_LISTING_CATEGORY ) ? __( 'category', APP_TD ) : __( 'tag', APP_TD ) ), single_term_title( '', false ) ); ?></h2>
	</article>
	<?php endif; ?>
<?php endif; ?>
	<div class="advert">
